...
def imdbscore():
    ...
    for item in data:
        score.append(item[0])
        num.append(item[1])
    sql2 = "select country,count(country) from movie250 group by country"  #得到国家电影数目的数据
    data2 = cur.execute(sql2)
    for item in data2:
        country.append(item)
    sql3 = "select year,count(year) from movie250 group by year"  # 得到国家电影数目的数据
    data3 = cur.execute(sql3)
    for item in data2:
        year.append(item[0])
        yearnum.append(item[1])
    sql4 = "select mvtype from movie250 "  # 得到国家电影数目的数据
...
def scorecompare():
    ...
    for mov in mvtype:
        mvtypenum.append(mvtypedata.count(mov))
    sql5 = "select score,rated from movie250 "  # 得到电影评价人数
    ...
    conn = sqlite3.connect("doubanmovie.db")
    cur = conn.cursor()
    Dsql = "select score,count(score) from movie250 group by score"
    Ddata = cur.execute(Dsql)
    for item in Ddata:
        Dscore.append(item[0])
        Dnum.append(item[1])
    Dsql2 = "select country,count(country) from movie250 group by country"  # 得到国家电影数目的数据
    Ddata2 = cur.execute(Dsql2)
    for item in Ddata2:
        Dcountry.append(item)
    Dsql3 = "select year,count(year) from movie250 group by year"  # 得到国家电影数目的数据
    Ddata3 = cur.execute(Dsql3)
    for item in Ddata2:
        Dyear.append(item[0])
        Dyearnum.append(item[1])
    Dsql4 = "select mvtype from movie250 "  # 得到国家电影数目的数据
    ...
    Dsql5 = "select score,rated from movie250 "  # 得到电影评价人数
...
def recommov():
    ...
    # 传递数据
    return render_template("recommovR.html", choose=choose)
...
def delRecommov(*args):  # *args代表一个元组
    ...
    yearChoosed = list(set(yearChoosed)) #去重
    print(sorted(yearChoosed))
    print(len(yearChoosed))

    finalChoosedmovies=[]

    for id in yearChoosed:

        finalChoosedmovies.append(movies[id-1])

    return finalChoosedmovies

@app.route('/recommovR')
def recommovR():

    finalChoosedmovies = delRecommov(choose)
    return  render_template("recommovR.html",finalChoosedmovies = finalChoosedmovies)
